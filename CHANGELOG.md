...
- **Verdict trägt jetzt `review_mode` + `reviewers[]`**, jeder Befund zusätzlich `source_lens`, `location` und `repro`. Ein `high`-Befund ohne `repro`-Befehl ist ein Verdacht, kein Befund. Ab Iteration 2 werden **nur die Linsen mit High-Findings** neu gestartet; triviale Flows gehen weiterhin über `approved-trivial` raus — ganz ohne Dispatch, aber erst wenn alle vier Skip-Bedingungen gelten (≤ 2 Tasks, keine Schema-Änderung, kein neuer Endpoint und kein Tag aus `force_critic_tags`, standardmäßig `security`/`breaking`).
...
Nebenbefund beim Regenerieren von `packages/skills/index.json`: Die Index-Zeile für `devflow-subagent-driven-dev` war an zwei Stellen abgedriftet — der Iron-Law-Zähler stand auf 5, während das Frontmatter längst 8 Gesetze führt, und die Beschreibung verwies noch auf die offene RFC-Diskussion, obwohl DF-427 sie am 2026-05-22 abgeschlossen hat. Der Rebuild zieht beides nach — der Skill selbst (weiterhin `status: draft`) wird von DF-535 nicht angefasst, nur seine abgedriftete Index-Zeile.
