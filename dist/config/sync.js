...
import { DEFAULT_CONFIG, DEFAULT_STRICTNESS, deriveEnforcementFromStrictness } from './types.js';
 ...
 */
async function saveCache(config) {
    try {
...
function parseRemoteConfig(raw) {
    ...
    if (raw.strictness && typeof raw.strictness === 'object') {
        ...
        if (raw.requiredFields && typeof raw.requiredFields === 'object') {
            config.requiredFields = raw.requiredFields;
        }
    }
    return config;
}
