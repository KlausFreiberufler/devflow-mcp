    ...
    for (const att of attachments) {
        ...
        if (INLINE_TEXT_MIME.has(mime) && inlineBudgetRemaining > 0) {
            ...
            if (contentResult.success && contentResult.text !== undefined) {
                ...
                inlineTextSections.push(`## Attachment: ${att.originalName}\n\n` +
                    '```' + lang + '\n' + slice + (truncated ? '\n... [truncated]' : '') + '\n```');
...
export async function handleFlowUpdate(args) {
    const flowId = args.flowId;
    ...
    if (currentState) {
        ...
        if (required) {
            ...
            const missing = required.fields.filter(f => !allArgs[f]);
            if (missing.length > 0) {
        ...
        if (currentState === 'review' && strictness.docsUpdate >= 3) {
            ...
            const existingCommits = flowData?.commits
                ? (typeof flowData.commits === 'string' ? JSON.parse(flowData.commits) : flowData.commits)
                : [];
            const allCommits = [...existingCommits, ...(commits || [])];
            const hasDocsCommit = allCommits.some((c) => /\bdocs?\b/i.test(c.message));
    ...
    const cleanUpdate = {};
    if (currentState)
        cleanUpdate.currentState = currentState;
    if (agentStatus)
        cleanUpdate.agentStatus = agentStatus;
    if (agentMessage)
        cleanUpdate.agentMessage = agentMessage;
    if (acceptanceCriteria)
        cleanUpdate.acceptanceCriteria = acceptanceCriteria;
    if (implementationPlan)
        cleanUpdate.implementationPlan = implementationPlan.replace(/\\n/g, '\n');
    if (agentSummary)
        cleanUpdate.agentSummary = agentSummary.replace(/\\n/g, '\n');
    if (testingInstructions)
        cleanUpdate.testingInstructions = testingInstructions.replace(/\\n/g, '\n');
    if (branchName)
        cleanUpdate.branchName = branchName;
    if (commits)
        cleanUpdate.commits = commits;
    if (prUrl)
        cleanUpdate.prUrl = prUrl;
    if (prNumber)
        cleanUpdate.prNumber = prNumber;
    if (prState)
        cleanUpdate.prState = prState;
    ...
    if (implementationPlan && result.success) {
        try {
            await devFlowClient.uploadAttachment(resolvedId, 'implementation-plan.md', implementationPlan.replace(/\\n/g, '\n'), 'text/markdown');
        }
        catch {
