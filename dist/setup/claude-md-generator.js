...
import { generateClaudeMdContent, generateGuidelinesBlock, MARKER_START, MARKER_END, GUIDELINES_MARKER_START, GUIDELINES_MARKER_END, } from '../templates/claude-md.js';
/**
 ...
 */
export async function setupClaudeMd(workingDir, projectName, techStack, strictness) {
    ...
    const hasMarkers = existingContent.includes(MARKER_START) &&
 ...
 */
export async function syncProjectGuidelines(workingDir, guidelines) {
    const claudeMdPath = join(workingDir, 'CLAUDE.md');
    let existingContent;
    try {
        existingContent = await readFile(claudeMdPath, 'utf-8');
    ...
    const hasMarkers = existingContent.includes(GUIDELINES_MARKER_START) &&
 ...
 */
export async function fetchProjectTechStack(baseUrl, token, projectId) {
    try {
