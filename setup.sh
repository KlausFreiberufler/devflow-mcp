...
echo ""
read -p "[2/3] WorkFlow Pro URL (default: http://localhost:6011): " WFP_URL
...
# 4. Cleanup: Remove old global symlink if present
echo ""
...
if [ -L "$CLAUDE_MD" ]; then
  ...
  if echo "$LINK_TARGET" | grep -q "workflow-pro-mcp"; then
    rm "$CLAUDE_MD"
