...
function parseUri(uri: string): { projectId: string; kind: string; arg?: string; query?: Record<string, string> } | null {
...
export async function readResource(uri: string): Promise<ResourceContent> {
  ...
  switch (p.kind) {
    case 'adr': {
      ...
      const a = r.data as { number: number; title: string; status: string; content: string };
      return {
        uri,
        mimeType: 'text/markdown',
    ...
    case 'flow': {
      ...
      const flow = (list.data as any[]).find(f => f.displayId === p.arg || f.id === p.arg);
    ...
    case 'graph': {
      ...
      const byType = g.nodes.reduce<Record<string, number>>((acc, n) => { acc[n.type] = (acc[n.type] || 0) + 1; return acc; }, {});
      return {
        uri,
        mimeType: 'text/markdown',
        text: [
