...
async function handle(args: Record<string, unknown>): Promise<string> {
  ...
  const subsystems = args.subsystems as string[] | undefined;
  const qs = subsystems?.length ? `?subsystems=${encodeURIComponent(subsystems.join(','))}` : '';
  const r = await (devFlowClient as any).request?.('GET', `/api/projects/${projectId}/bootstrap-audit/prepare${qs}`);
  if (!r?.success || !r?.data) return `Error: ${r?.error || 'prepare failed'}`;
  return (r.data as any).instructions || 'No instructions returned.';
}
