...
async function handleProjectGuidelinesGet(args: Record<string, unknown>): Promise<string> {
  ...
  return formatGuidelines(result.data as ProjectGuidelines, projectId);
}

async function handleProjectGuidelinesUpdate(args: Record<string, unknown>): Promise<string> {
  const projectId = (args.projectId as string | undefined) || devFlowClient.getLinkedProjectId();
  const guidelines = args.guidelines as string;
...
function formatGuidelines(data: ProjectGuidelines, projectId?: string): string {
  const lines = [
    '# Project Guidelines',
    '',
  ];
