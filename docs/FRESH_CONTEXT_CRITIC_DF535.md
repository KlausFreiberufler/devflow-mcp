...
Dispatch **2–3 reviewer subagents** (Claude Code `Agent`/Task tool), one lens each, in parallel — one message, multiple tool calls. Iteration 1 always dispatches all three; a smaller dispatch only happens on [re-dispatch](#iteration), where just the lenses with prior high-findings run again. That is where the lower end of “2–3” comes from — never a free choice in iteration 1. Never pick an agent type that carries `Edit`/`Write` — neither `feature-dev:code-reviewer` nor `Explore` does, so both are eligible. They read the repo themselves; they report, they never fix.
...
Each finding a lens reports carries `severity`, `source_lens`, `location`, `observation`, `repro` and `suggestion`. When the author folds those into the skill's own verdict, `observation` maps onto the pre-existing `issue` field and the author adds `dimension`. The keys that existed before DF-535 — `severity`, `dimension`, `issue`, `suggestion` — keep their names and meaning, so `observation` is never emitted at the top level; DF-535 only *adds* `source_lens`, `location` and `repro` to `findings[]`. `source_lens` names the lens that produced it — or `author` for the two dimensions no subagent ever sees (3 and 6), so a verdict stays traceable to who found what. A `high` finding without a `repro` command is a suspicion, not a finding — the lens gets sent back for it.

In each dispatched iteration every lens gets an entry in `reviewers[]` — `lens`, `agent_type`, `status`, `findings_count`. `status` is `returned` once the lens crossed the boundary and reported. A lens that errors out or returns nothing usable is `status: "failed"` — re-dispatch it once. If it fails again, the author covers that lens' dimensions themselves and the entry stays in `reviewers[]` marked `failed`. `failed` and `skipped` entries additionally carry a `reason`, so the gap stays readable from the JSON alone. `review_mode` stays `fresh-context` as long as at least one lens actually crossed the boundary: the field says how the review was produced, not how completely.
